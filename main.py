...
def motor_operations(motor):
    ...
    if st.session_state.angle is not None:
        motor.move_to_angle(st.session_state.angle)
...
def main():
    ...
    # Start motor operations in a separate thread
    motor_thread = threading.Thread(target=motor_operations, args=(motor,))
    motor_thread.start()

    # Continuously display the current angle on the Streamlit page
